...
class AdventOfCodeHelpers
  ...
  def self.get_input(day)
    ...
    if File.exist?(file_name)
      ...
      res = Net::HTTP.get_response(URI("https://adventofcode.com/#{ENV["AOC_YEAR"] || 2018}/day/#{day}/input"), { Cookie: "session=#{ENV["AOC_SESSION"]}", "User-Agent" => "Ben McHone <ben@mchone.dev>" })
      body = res.body
  ...
  def self.can_submit?(day, level)
    ...
    body = Net::HTTP.get(URI("https://adventofcode.com/#{ENV["AOC_YEAR"] || 2018}/day/#{day}"), { Cookie: "session=#{ENV["AOC_SESSION"]}", "User-Agent" => "Ben McHone <ben@mchone.dev>" })
  ...
  def self.submit_answer(day, answer, level)
    ...
    res = Net::HTTP.post(URI("https://adventofcode.com/#{ENV["AOC_YEAR"] || 2018}/day/#{day}/answer"), "level=#{level}&answer=#{answer}", { Cookie: "session=#{ENV["AOC_SESSION"]}", "User-Agent" => "Ben McHone <ben@mchone.dev>" })
